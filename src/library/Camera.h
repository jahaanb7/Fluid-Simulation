...
class Camera{
  ...
  void moveZ(float amount){camPosition += kHat * amount;}
  void moveX(float amount) {camPosition += iHat * amount;}
  void moveY(float amount) {camPosition += jHat * amount;}

  glm::vec3 get_kHat(){return kHat;}
  glm::vec3 get_iHat(){return iHat;}
  glm::vec3 get_jHat(){return jHat;}

  glm::vec3 getPosition() {return glm::vec3(camPosition.x, camPosition.y, camPosition.z);}
  ...
  void MoveCamera(GLFWwindow* window, float deltaTime){
    ...
    if(!mouseLock){
      ...
      if(glfwGetKey(window, GLFW_KEY_A) == GLFW_PRESS){moveX(-moveSpeed);}
      if(glfwGetKey(window, GLFW_KEY_D) == GLFW_PRESS){moveX(moveSpeed);}
      if(glfwGetKey(window, GLFW_KEY_Q) == GLFW_PRESS){moveY(moveSpeed);}
      if(glfwGetKey(window, GLFW_KEY_E) == GLFW_PRESS){moveY(-moveSpeed);}
